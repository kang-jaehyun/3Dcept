...
class SemSegEvaluator(HookBase):
    ...
    def eval(self):
        ...
        if self.trainer.cfg.data.train.type =="ConcatDataset" and self.trainer.cfg.data.val.type == 'ScanNetPlusPlusDataset':
            ...
            self.trainer.logger.info(
                "Val result: mIoU_top100/mAcc_top100 {:.4f}/{:.4f}.".format(
                    top100_m_iou, top100_m_acc
                )
            )

            current_epoch = self.trainer.epoch + 1
            for ind in TOP100_CLASS_INDEX:
                self.trainer.logger.info(
                    "Class_{idx}-{name} Result: iou/accuracy {iou:.4f}/{accuracy:.4f}".format(
                        idx=ind,
                        name=self.trainer.cfg.data.names[ind],
                        iou=iou_class[ind],
                        accuracy=acc_class[ind],
                    )
                )
                if self.trainer.writer is not None:
            ...
            if self.trainer.writer is not None:
                ...
                self.trainer.writer.add_scalar("val/mIoU", m_iou, current_epoch)
                self.trainer.writer.add_scalar("val/mAcc", m_acc, current_epoch)
                self.trainer.writer.add_scalar("val/mIoU_top100", top100_m_iou, current_epoch)
                self.trainer.writer.add_scalar("val/mAcc_top100", top100_m_acc, current_epoch)
                self.trainer.writer.add_scalar("val/allAcc", all_acc, current_epoch)
            self.trainer.logger.info("<<<<<<<<<<<<<<<<< End Evaluation <<<<<<<<<<<<<<<<<")
            self.trainer.comm_info["current_metric_value"] = top100_m_iou  # save for saver
            self.trainer.comm_info["current_metric_name"] = "mIoU_top100"  # save for saver
