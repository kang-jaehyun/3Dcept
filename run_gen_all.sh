...
slurm_setting_a100="-p suma_a100 -q a100_qos"
...
export node_idx=0
sbatch --gres=gpu:$num_gpus --cpus-per-task=8 $slurm_setting_a100 -J $JNAME --time=$DURATION ./${SCRIPT_DIR}/run_gen_master_node.sh
sleep 10
...
do
  echo "node $i"
  export node_idx=$i
  sbatch --gres=gpu:$num_gpus --cpus-per-task=8 $slurm_setting_a100 -J $JNAME --time=$DURATION ./${SCRIPT_DIR}/run_gen_slave_node.sh
done
